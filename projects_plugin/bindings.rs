pub mod phylum {
  ...
  where U: Phylum, T: Send,
  {
    use wit_bindgen_wasmtime::rt::get_memory;
    use wit_bindgen_wasmtime::rt::get_func;
    linker.func_wrap1_async("phylum", "projects", move |mut caller: wasmtime::Caller<'_, T>,arg0:i32| {
      Box::new(async move {
        ...
        let result = host.projects().await;
        let vec3 = result;
        let len3 = vec3.len() as i32;
        let result3 = func_canonical_abi_realloc.call_async(&mut caller, (0, 0, 4, len3 * 16)).await?;
        for (i, e) in vec3.into_iter().enumerate() {
          let base = result3 + (i as i32) * 16;
          {
            let Project{ name:name0, id:id0, } = e;
            let vec1 = name0;
            let ptr1 = func_canonical_abi_realloc.call_async(&mut caller, (0, 0, 1, vec1.len() as i32)).await?;
            ...
            let vec2 = id0;
            let ptr2 = func_canonical_abi_realloc.call_async(&mut caller, (0, 0, 1, vec2.len() as i32)).await?;
            let caller_memory = memory.data_mut(&mut caller);
